...
def load_data(data_path: str) -> Tuple[np.ndarray, np.ndarray]:
    ...
    print(
        f"Dados carregados: {X.shape[0]} amostras, {X.shape[1]} features, {y.shape[1]} classes"
    )
    return X, y
...
def create_model(input_dim: int, hidden: int, num_classes: int, seed: int) -> SimpleMLP:
    ...
    """
    model = SimpleMLP(
        input_dim=input_dim, hidden=hidden, num_classes=num_classes, seed=seed
    )
    ...
    for _ in range(num_replay_steps):
        if len(replay_buffer) >= replay_batch_size:
            X_replay, y_replay = replay_buffer.sample(
                replay_batch_size, strategy="uniform"
            )
...
def save_checkpoint(
    ...
    np.savez_compressed(
        model_path, **{f"{k}_{epoch}": v for k, v in model_params.items()}
    )
...
def main():
    ...
    # Parâmetros de dados
    parser.add_argument(
        "--data", type=str, required=True, help="Caminho para arquivo de dados (.npz)"
    )

    # Parâmetros do modelo
    parser.add_argument(
        "--hidden", type=int, default=64, help="Número de neurônios na camada oculta"
    )
    ...
    parser.add_argument(
        "--replay_capacity", type=int, default=1000, help="Capacidade do replay buffer"
    )

    parser.add_argument(
        "--replay_batch", type=int, default=32, help="Tamanho do batch de replay"
    )
    ...
    # Parâmetros de EWC
    parser.add_argument(
        "--use_ewc", type=str, default="False", help="Usar EWC (True/False)"
    )

    parser.add_argument(
        "--ewc_lambda", type=float, default=100.0, help="Peso do penalty EWC"
    )
    ...
    for epoch in range(args.epochs):
        ...
        loss, accuracy = train_epoch(
            model, optimizer, X, y, args.batch_size, replay_buffer, ewc, args.dp_sigma
        )
        ...
        if args.verbose or (epoch + 1) % 5 == 0:
            print(
                f"Época {epoch + 1:2d}/{args.epochs}: "
        ...
        if (epoch + 1) % args.checkpoint_freq == 0:
            save_checkpoint(
                model, optimizer, replay_buffer, ewc, epoch + 1, artifacts_dir
            )
