        ...
        for X, y in tasks:
            ...
            for name, param in self.model.named_parameters():
                if name in final_params:
                    param.data += self.meta_lr * (final_params[name] - initial_params[name])
...
def main():
    parser = argparse.ArgumentParser(description="Reptile Meta-Learning para Few-Shot Learning")
    ...
    parser.add_argument("--meta_epochs", type=int, default=100, 
                       help="Número de épocas meta-learning")
    parser.add_argument("--tasks_per_epoch", type=int, default=10, 
                       help="Número de tarefas por época")
    parser.add_argument("--shots_per_class", type=int, default=5, 
    ...
    for epoch in range(args.meta_epochs):
        ...
        tasks = generate_few_shot_tasks(
            X, y, args.tasks_per_epoch, args.shots_per_class, 
            args.query_per_class, num_classes, args.seed + epoch
        )
        ...
        if (epoch + 1) % args.eval_every == 0:
            eval_stats = evaluate_model(
                model, tasks, args.inner_steps, args.inner_lr
            )
            eval_history.append({
                'epoch': epoch,
                **eval_stats
            ...
            if args.verbose:
                print(f"Epoch {epoch+1:3d}/{args.meta_epochs} | "
                      f"Meta Loss: {meta_loss:.4f} | "
                      f"Accuracy: {eval_stats['mean_accuracy']:.3f} ± {eval_stats['std_accuracy']:.3f}")
        
        # Progress indicator
        if (epoch + 1) % 10 == 0:
            print(f"✅ Epoch {epoch+1:3d}/{args.meta_epochs} | Meta Loss: {meta_loss:.4f}")
    ...
    final_tasks = generate_few_shot_tasks(
        X, y, 50, args.shots_per_class, args.query_per_class, 
        num_classes, args.seed + 999
    )
    final_eval = evaluate_model(model, final_tasks, args.inner_steps, args.inner_lr)
    
    print(f"\n📈 Resultados Finais:")
    print(f"   Accuracy: {final_eval['mean_accuracy']:.3f} ± {final_eval['std_accuracy']:.3f}")
    print(f"   Range: [{final_eval['min_accuracy']:.3f}, {final_eval['max_accuracy']:.3f}]")
