...
class SecurePrivacyDemo:
    ...
    def setup_clients(self) -> Dict[str, str]:
        ...
        for i in range(self.num_clients):
            ...
            print(f"   ✅ Cliente {client_id} registrado com token: {session_token[:8]}...")
        
        return self.client_sessions
    
    def generate_client_updates(self, input_dim: int = 20, hidden_dim: int = 64) -> Dict[str, Dict[str, np.ndarray]]:
        ...
        for client_id in self.client_sessions.keys():
            ...
            print(f"   ✅ {client_id}: update gerado com {sum(v.size for v in update.values())} parâmetros")
        
        return self.client_updates
    
    def apply_differential_privacy(self, clip_norm: float = 1.0) -> Dict[str, Dict[str, np.ndarray]]:
        """Apply differential privacy to client updates"""
        print(f"🔒 Aplicando Differential Privacy (ε={self.privacy_epsilon}, δ={self.privacy_delta})...")
        ...
        for client_id, update in self.client_updates.items():
            ...
            print(f"   ✅ {client_id}: privacidade aplicada (sensitivity={sensitivity:.3f})")
        
        return private_updates
    
    def submit_secure_updates(self, private_updates: Dict[str, Dict[str, np.ndarray]]) -> bool:
    ...
    def perform_secure_aggregation(self) -> Optional[Dict[str, np.ndarray]]:
        ...
        print(f"   📊 Status: {status['submitted_updates']}/{status['threshold']} updates prontos")
        ...
        if aggregated_update:
            ...
            print(f"   📊 Resultado: {sum(v.size for v in aggregated_update.values())} parâmetros")
        else:
            print(f"   ❌ Falha na agregação")
        
        return aggregated_update
    
    def verify_results(self, aggregated_update: Dict[str, np.ndarray]) -> Dict[str, Any]:
        ...
        utility_metrics = self._calculate_utility_metrics(original_updates, aggregated_update)
    ...
    def _calculate_utility_metrics(
        ...
        try:
            expected_flat = np.concatenate([v.flatten() for v in expected_aggregated.values()])
            actual_flat = np.concatenate([v.flatten() for v in aggregated_update.values()])
    ...
    def run_demo(self, input_dim: int = 20, hidden_dim: int = 64, clip_norm: float = 1.0) -> Dict[str, Any]:
...
def main():
    parser = argparse.ArgumentParser(description="Demo de Segurança e Privacidade para Federated Learning")
    parser.add_argument("--clients", type=int, default=5, 
                       help="Número de clientes federados")
    parser.add_argument("--epsilon", type=float, default=1.0, 
                       help="Parâmetro de privacidade epsilon")
    parser.add_argument("--delta", type=float, default=1e-5, 
                       help="Parâmetro de privacidade delta")
    ...
    parser.add_argument("--clip_norm", type=float, default=1.0, 
                       help="Norma para clipping de gradientes")
    ...
    if results:
        ...
        print(f"   Epsilon usado: {results['privacy_status']['total_epsilon_used']:.3f}")
        print(f"   Epsilon restante: {results['privacy_status']['remaining_epsilon']:.3f}")
    else:
