...
class ONNXRuntimeDemo:
    ...
    def predict_batch(
        self, input_batch: np.ndarray, batch_size: int = 32
    ) -> np.ndarray:
...
def create_synthetic_data(
    num_samples: int, input_dim: int, num_classes: int, seed: int = 42
...
def main():
    ...
    parser.add_argument(
        "--onnx_model", type=str, required=True, help="Caminho para modelo ONNX"
    )
    ...
    parser.add_argument(
        "--num_runs", type=int, default=100, help="Número de execuções para benchmark"
    )
    parser.add_argument(
        "--compare", action="store_true", help="Comparar com modelo PyTorch"
    )
    parser.add_argument(
        "--interactive", action="store_true", help="Executar demo interativo"
    )
    parser.add_argument(
    ...
    X, y = create_synthetic_data(
        args.num_samples, args.input_dim, args.num_classes, args.seed
    )
    ...
    print(
        f"   Tempo médio por inferência: {benchmark_results['avg_time_per_inference']*1000:.3f} ms"
    )
    print(
        f"   Throughput: {benchmark_results['throughput_samples_per_second']:.0f} amostras/s"
    )
    ...
    if args.compare:
        ...
        perf_comparison = benchmark_comparison(
            pytorch_model, onnx_demo, X[:100], args.num_runs
        )

        print(f"\n📊 Comparação de Performance:")
        print(
            f"   PyTorch: {perf_comparison['pytorch']['avg_time_per_inference']*1000:.3f} ms"
        )
        print(
            f"   ONNX:    {perf_comparison['onnx']['avg_time_per_inference']*1000:.3f} ms"
        )
