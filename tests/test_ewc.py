...
import numpy as np
from src.ewc import (
    EWC, OnlineEWC, compute_fisher_diag, 
    compute_ewc_loss, analyze_parameter_importance
)
    ...
    def test_update_fisher_online(self):
        ...
        for param_name in ewc.fisher_online:
            assert not np.array_equal(
                ewc.fisher_online[param_name], 
                fisher_initial[param_name]
            )
...
class TestEWCIntegration:
    ...
    def test_ewc_with_optimizer(self):
        ...
        for param_name in current_params:
            current_params[param_name] += 0.1 * np.random.randn(*current_params[param_name].shape)
