...
class PrivacyBudget:
    ...
    def __post_init__(self):
        ...
        if self.epsilon > 10:
            print("Warning: Epsilon > 10 may provide weak privacy guarantees")
...
class DifferentialPrivacy:
    ...
    def add_noise(self, value: Union[float, np.ndarray], sensitivity: float) -> Union[float, np.ndarray]:
    ...
    def _add_laplace_noise(self, value: Union[float, np.ndarray], sensitivity: float) -> Union[float, np.ndarray]:
    ...
    def _add_gaussian_noise(self, value: Union[float, np.ndarray], sensitivity: float) -> Union[float, np.ndarray]:
    ...
    def _add_exponential_noise(self, value: Union[float, np.ndarray], sensitivity: float) -> Union[float, np.ndarray]:
    ...
    def get_privacy_cost(self, num_queries: int = 1) -> Dict[str, float]:
        ...
        if self.mechanism == NoiseMechanism.LAPLACE:
            ...
            total_epsilon = self.privacy_budget.epsilon * math.sqrt(2 * num_queries * math.log(1 / self.privacy_budget.delta))
            total_delta = self.privacy_budget.delta * num_queries
        else:
        ...
        return {
            ...
            'remaining_epsilon': max(0, 10 - total_epsilon),  # Assuming max budget of 10
...
def test_differential_privacy(
    ...
    """Test differential privacy mechanisms on synthetic data"""
    results = {}
    
    # Test different noise mechanisms
    for mechanism in NoiseMechanism:
        privacy_budget = PrivacyBudget(epsilon, delta, mechanism)
        ...
        noise_level = np.std(noisy_data - data)
        
        results[mechanism.value] = {
            'original_mean': original_mean,
            'noisy_mean': noisy_mean,
            'noise_level': noise_level,
            'privacy_cost': dp.get_privacy_cost(1)
        }
