...
class ReplayBuffer:
    ...
    def sample(
        ...
        if batch_size > self.size:
            ...
            print(
                f"Warning: batch_size reduzido para {batch_size} (tamanho disponível)"
            )
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "avg_importance": (
                np.mean(list(self.importance_scores)) if self.size > 0 else 0.0
            ),
            "min_importance": (
                np.min(list(self.importance_scores)) if self.size > 0 else 0.0
            ),
            "max_importance": (
                np.max(list(self.importance_scores)) if self.size > 0 else 0.0
            ),
        }
