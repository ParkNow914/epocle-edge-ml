...
class HomomorphicEncryption:
    ...
    def decrypt(self, encrypted_value: int) -> float:
        ...
        if hasattr(encrypted_value, "item"):
            encrypted_value = encrypted_value.item()
...
class SecureAggregator:
    ...
    def submit_update(
        self, client_id: str, session_token: str, model_update: Dict[str, np.ndarray]
    ) -> bool:
    ...
    def _homomorphic_aggregate(
        self, encrypted_updates: List[SecureUpdate]
    ) -> Dict[str, Any]:
    ...
    def _decrypt_aggregated(
        ...
        for key, value in aggregated_update.items():
            if isinstance(value, dict):
                ...
                decrypted_update[key] = self._decrypt_aggregated(value)
            elif isinstance(value, (list, np.ndarray)):
                # Decrypt array - convert to numpy array first
                if isinstance(value, list):
                    value = np.array(value)
                # Apply decrypt to each element
                decrypted_array = np.vectorize(self.encryption.decrypt)(value)
                decrypted_update[key] = decrypted_array
            elif isinstance(value, bytes):
                # Handle bytes (mock data in tests) - convert to int hash
                decrypted_update[key] = hash(value) % 1000 / 1000.0
            else:
                # Decrypt scalar
                try:
                    decrypted_update[key] = self.encryption.decrypt(value)
                except (TypeError, ValueError):
                    # Handle mock data in tests - convert to float
                    decrypted_update[key] = float(hash(str(value)) % 1000) / 1000.0
...
def secure_model_update_aggregation(
    client_updates: List[Tuple[str, Dict[str, np.ndarray]]], threshold: int = None
    ...
    for client_id, model_update in client_updates:
        # Register client directly with the security instance
        session_token = security.register_federated_client(client_id)

        # Submit update
        success = security.submit_federated_update(
            client_id, session_token, model_update
        )
        if not success:
...
def generate_test_model_update(
